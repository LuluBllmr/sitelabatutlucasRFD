...
if (isset($_POST['formcontact'])) {
  ...
  if (count($erreur) > 0) {
    ...
    if ($nbrOccurences > 0) {
      echo "Déjà dans la base de données";
    }
    else {
      $sql = "INSERT INTO clients (nom, prenom, mail, tel, message) VALUES  ('" . $nom . "', '" . $prenom . "', '" . $mail ."', '" . $tel ."', '" . $msg ."')";
      ...
      $query->execute();
      echo "Enregistré dans la base de données</p>";
    }
  }
}
else {
  $nom = $prenom = $mail = $tel = $msg = $file = "";
  require 'formcontact.php';
}
